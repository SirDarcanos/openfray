...
`print/` compiles the PDF with [Typst](https://typst.app), from the web edition's MDX and
the same creature JSON the console fetches — full detail in
[`print/README.md`](./print/README.md). The rule to know before touching it:

> **Never edit `print/chapters/*.typ`.** They are generated by
> `scripts/generate-print.mjs` and overwritten. Edit the MDX.

The pipeline is three layers, because the two editions share a **text** but not a
**running order**:

- `print/chapters/*.typ` — generated prose and creature roster.
- `print/waking-garden.typ` — hand-authored, and owns arrangement alone: which pages
  exist, in what order, plus the cover, contents, indexes, console spread and licensing
  placement. Print has matter the web has no equivalent for, and the reverse.
- `print/bestiary.typ` — the layout, reusable across libraries.

`npm run build` fails if the generated chapters are stale against their sources, in the
same family as the spacing and specificity guards. It runs on Node alone, so **Typst is
not needed to build the site** — only to produce the PDF (`npm run print`).

Two print/web differences are deliberate and encoded rather than hand-maintained: the
web's "library" becomes "book" via a whole-word map that **asserts its occurrence count**
(so a copy edit that changes it fails the build instead of silently rewriting prose), and
creature links become page references resolved from labels on each stat block.
