...
2. **Net the adv/disadv:** any number of advantage sources + any number of
   ...
   straight roll** (5e rule). Result records _why_.
...
```jsonc
{
  ...
  "applied": [
    // transparency: what modified this roll & why
...
- Adv/disadv auto-applied from effects shows as a small badge on the result
  ("ADV — Reckless Attack") so the DM sees _why_ and can override with one tap.
...
- For skill checks/saves of a _creature_: tap the creature → its saves/skills are
...
- **Show both dice on adv/disadv**, dim the discarded one — players want to see
  the 18 it kept _and_ the 4 it dropped.
