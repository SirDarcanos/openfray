...
spell is unlimited, while a _2/Day Each_ spell counts down per spell, so burning a Fireball
leaves its Invisibility untouched. It grays out when it's spent.
...
still cast it, and you type the numbers yourself. That's the path for a player's spell,
...
Spells that only affect the caster (_Range: Self_, like Speak with Animals) go straight onto them, with no
...
marks the caster as concentrating **once the spell lands**, meaning a target failed, the attack
hit, or you applied its effect. The timer is already counting. A spell everyone shrugs
