...
export function CustomMonsterForm({
  ...
  const hpAvg = averageHp(
    Number(d.hpDieCount) || 0,
    Number(d.hpDie) || 0,
    Math.floor(Number(d.hpMod) || 0),
  )
          ...
          >
            <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-slate-800">
              <h2 className="text-lg font-semibold">
                {editId ? 'Edit creature' : 'Custom creature'}
              </h2>
              <button
                type="button"
                onClick={onClose}
            ...
            <div className="max-h-[70vh] space-y-3 overflow-auto p-4">
              <Section title="Identity" open>
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  <select
                    value={d.size}
                    onChange={(e) => patch({ size: e.target.value as MonsterDraft['size'] })}
                    aria-label="Size"
                    className={FIELD}
                  >
                    {SIZES.map((s) => (
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  <select
                    value={d.edition}
                    onChange={(e) => patch({ edition: e.target.value as MonsterDraft['edition'] })}
                    aria-label="Edition"
                    className={FIELD}
                  >
              ...
              <Section title="Defense & HP" open>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <input
                    value={d.hpDieCount}
                    onChange={(e) => patch({ hpDieCount: e.target.value })}
                    placeholder="#"
                  ...
                  <select
                    value={d.hpDie}
                    onChange={(e) => patch({ hpDie: e.target.value })}
                  ...
                  <input
                    value={d.hpMod}
                    onChange={(e) => patch({ hpMod: e.target.value })}
                    placeholder="mod"
                    aria-label="Hit points modifier"
                    inputMode="numeric"
                    className={`${FIELD_W} w-16`}
                  />
              ...
              <Section title="Abilities & saves" open>
                  ...
                  {ABILITIES.map((a) => (
                    <div key={a} className="space-y-1">
                      ...
                      <input
                        value={d.abilities[a]}
                        onChange={(e) =>
                          patch({ abilities: { ...d.abilities, [a]: e.target.value } })
                        }
                        aria-label={`${a} score`}
                        inputMode="numeric"
                      ...
                      <label className="flex items-center justify-center gap-1 text-[11px] text-slate-500 dark:text-slate-400">
                        <input
                          type="checkbox"
                          checked={d.saves[a]}
                          onChange={(e) => patch({ saves: { ...d.saves, [a]: e.target.checked } })}
                          aria-label={`${a} save proficient`}
                        />
                        Proficient
                      </label>
                    </div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Enter each ability score; tick “Proficient” for a proficient save — the bonus is
                  derived from the modifier + proficiency bonus (set the CR above).
                </p>
              </Section>
              ...
              <Section title="Skills, senses & languages">
                <div className="space-y-1">
                  <p className={LABEL}>
                    Skills (proficient — bonus derived; tick expertise to double it)
                  </p>
                  {d.skills.map((s) => (
                    <div key={s.id} className="flex items-center gap-2">
                        ...
                        onChange={(e) =>
                          patch({
                            skills: d.skills.map((x) =>
                              x.id === s.id ? { ...x, skill: e.target.value as typeof x.skill } : x,
                            ),
                          })
                        }
                        aria-label="Skill"
                      ...
                      <select
                        value={s.expertise ? 'expertise' : 'proficient'}
                        onChange={(e) =>
                          patch({
                            skills: d.skills.map((x) =>
                              x.id === s.id
                                ? { ...x, expertise: e.target.value === 'expertise' }
                                : x,
                            ),
                          })
                        }
                        aria-label="Proficiency"
                        className={`${FIELD_W} w-32 shrink-0`}
                      >
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  <input
                    value={d.senses.passivePerception}
                    onChange={(e) =>
                  ...
                  />
                  <input
                    value={d.senses.darkvision}
                    onChange={(e) => patch({ senses: { ...d.senses, darkvision: e.target.value } })}
                    placeholder="Darkvision"
                    aria-label="Darkvision"
                    inputMode="numeric"
                    className={FIELD}
                  />
                  <input
                    value={d.senses.blindsight}
                    onChange={(e) => patch({ senses: { ...d.senses, blindsight: e.target.value } })}
                    placeholder="Blindsight"
                    aria-label="Blindsight"
                    inputMode="numeric"
                    className={FIELD}
                  />
                  <input
                    value={d.senses.tremorsense}
                    onChange={(e) =>
                      patch({ senses: { ...d.senses, tremorsense: e.target.value } })
                    }
                    placeholder="Tremorsense"
                    aria-label="Tremorsense"
                    inputMode="numeric"
                    className={FIELD}
                  />
                  <input
                    value={d.senses.truesight}
                    onChange={(e) => patch({ senses: { ...d.senses, truesight: e.target.value } })}
                    placeholder="Truesight"
                    aria-label="Truesight"
                    inputMode="numeric"
                    className={FIELD}
                  />
                </div>
              ...
              <Section title="Legendary & lair">
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <label className="text-sm" htmlFor="legendary-per-round">
                      Legendary actions per round
                    </label>
                    <input
                      id="legendary-per-round"
                      value={d.legendaryPerRound}
                      onChange={(e) => patch({ legendaryPerRound: e.target.value })}
                      aria-label="Legendary actions per round"
                      inputMode="numeric"
                      className={`${FIELD_W} w-20`}
                    />
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <input
                    value={d.legendaryResistance}
                    onChange={(e) => patch({ legendaryResistance: e.target.value })}
                    placeholder="Legendary Resistance / day"
                    aria-label="Legendary Resistance per day"
                    inputMode="numeric"
                    className={FIELD}
                  />
                  <input
                    value={d.legendaryResistanceLair}
                    onChange={(e) => patch({ legendaryResistanceLair: e.target.value })}
                    placeholder="…in lair"
                    aria-label="Legendary Resistance in lair"
                    inputMode="numeric"
                    className={FIELD}
                  />
                </div>
              ...
              <Section title="Spellcasting">
                <div className="flex flex-wrap items-center gap-2">
                  <select
                    ...
                    onChange={(e) =>
                      patch({ spellAbility: e.target.value as MonsterDraft['spellAbility'] })
                    }
                  ...
                  {spellBonus != null && (
                    <span className="text-xs text-slate-500 dark:text-slate-400 tabular-nums">
                      ...
                      <span className="font-medium">
                        {spellBonus >= 0 ? `+${spellBonus}` : spellBonus}
                      </span>{' '}
                ...
                <div className="space-y-2">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <select
                          ...
                          onChange={(e) =>
                            patch({
                              spellGroups: d.spellGroups.map((x) =>
                                x.id === g.id
                                  ? { ...x, usage: e.target.value as typeof x.usage }
                                  : x,
                              ),
                            })
                          }
                          aria-label="Spell usage"
                        ...
                        {g.usage === 'perDay' && (
                          <input
                            ...
                            onChange={(e) =>
                              patch({
                                spellGroups: d.spellGroups.map((x) =>
                                  x.id === g.id ? { ...x, per: e.target.value } : x,
                                ),
                              })
                            }
                            placeholder="N"
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      patch({ spellGroups: [...d.spellGroups, emptySpellGroupDraft()] })
                    }
                    className="text-xs text-indigo-600 hover:underline dark:text-indigo-400"
                  >
                    + Add spell group
