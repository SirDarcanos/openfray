...
// (wakingGarden.ts, broodAndBloom.ts) holds only its dataset and lookup.

/** Case-insensitive creature lookup over one book's dataset. Throws at build time on a
 *  typo, which is the point — a missing entry must fail the build, not render a blank
 *  card. */
export function creatureLookup(creatures: Creature[], book: string): (name: string) => Creature {
  const byName = new Map(creatures.map((c) => [c.name.toLowerCase(), c]));
  return (name) => {
    const found = byName.get(name.toLowerCase());
    if (!found) throw new Error(`${book}: no creature named "${name}"`);
    return found;
  };
}
