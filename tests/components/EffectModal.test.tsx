...
describe('EffectModal', () => {
  it('applies a condition with the chosen duration', () => {
    const onApply = vi.fn()
    render(<EffectModal name="Goblin" onApply={onApply} />)
    const dialog = open()
  ...
  it('builds a save-ends duration', () => {
    const onApply = vi.fn()
    render(<EffectModal name="Goblin" onApply={onApply} />)
    const dialog = open()
    ...
    expect(onApply.mock.calls[0][0].duration).toEqual({ type: 'saveEnds', save: { ability: 'wis', dc: 15 } })
  })
})
